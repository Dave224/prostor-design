...
const vueCompiler = require("gulp-vue-single-file-component");
...
function scriptsVue() {
    return src(path.components + "**/*.vue")
        .pipe(plumber())
        .pipe(clip())
        .pipe(vueify({
            extractCSS: false
        ...
        .pipe(concat("vueComponents.js"))
        // .pipe(dest(path.vueComponents));
        .pipe(dest(path.build))
        .pipe(touch());

}
exports.scriptsVue = scriptsVue;
...
function watchAdminFiles() {
    ...
    // watch(path.components + "**/*.vue", series(scriptsVue, scriptsBundle));
...
const buildJs = parallel(scripts, scriptsVendors, scriptsVue);
