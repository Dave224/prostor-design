...
use Layouts\Block\BlockConfig;

$Input = BlockConfig::BLOCK_INPUT;
$FormPrefix = BlockConfig::FORM_PREFIX;

if (Admin::isPostType(Product::KEY)) {
    $Input = ProductConfig::BLOCK_INPUT;
    $FormPrefix = ProductConfig::FORM_PREFIX;
}

global $post;
$BlocksOrder = get_post_meta($post->ID, $Input);
...
<div id="blocks-field">
    ...
    <input type="text" v-model="BlocksOrder" name="<?= $FormPrefix; ?>[<?= $Input; ?>]">
        ...
        methods: {
            ...
            getBlocksAll() {
                axios.get(BriloRestApiUrl.url + "blocks")
                    ...
                    .then(function() {
                        new Selectr("#blocks-field select", {
                            width: 0
                        });
                    });
            },
