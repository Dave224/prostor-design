...
class KT_Form_Fieldset extends KT_HTML_Tag_Base implements ArrayAccess
{
     ...
     */
    public function addSwitch($name, $label, $labelYes = "ANO", $labelNo = "NE")
    {
        $field = $this->fields[$name] = new KT_Switch_Field($name, $label, $labelYes, $labelNo);
        $field->setPostPrefix($this->getPostPrefix());
    ...
    public function addTrumbowygTextareaMinimal($name, $label)
    {
        $field = $this->fields[$name] = new KT_Textarea_Field($name, $label);
        $field->setPostPrefix($this->getPostPrefix());
        $field->addAttrClass("fancy-textarea-minimal");
        $field->setHtmlDecode(true);
        return $field;
    }
